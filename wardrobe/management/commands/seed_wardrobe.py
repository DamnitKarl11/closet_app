...
class Command(BaseCommand):
    help = 'Seeds the database with realistic wardrobe items for the current user'

    def handle(self, *args, **kwargs):
        # Get the current user
        try:
            user = User.objects.get(username='admin')  # or whatever your default user is
        except User.DoesNotExist:
            self.stdout.write(self.style.ERROR('Default user not found. Please create a user first.'))
            return
        ...
        self.stdout.write(self.style.SUCCESS(f'Successfully created {len(ITEMS)} clothing items for user {user.username}')) 